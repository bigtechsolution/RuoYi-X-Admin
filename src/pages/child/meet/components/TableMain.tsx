...
import {getRecruitMeetList, getTherapistList, getTherapyList,} from '@/services/system/System';
  ...
  return [
    ...
    },
    {
      dataIndex: 'child', key: 'child', title: '아동명', valueType: 'text', render: (_, record) =>
        record.recruit.child ? (
          <>
            {record.recruit.child.name}
          </>
        ) : (
          <>-</>
        ),
    ...
    {
      dataIndex: 'phone', key: 'phone', title: '연락처', valueType: 'text', render: (_, record) =>
        record.recruit.owner ? (
          <>
            {record.recruit.owner.phone}
          </>
        ) : (
          <>-</>
        ),
    },
    {
      dataIndex: 'phone', key: 'phone', title: '연락처', valueType: 'text', render: (_, record) =>
        record.recruit.child ? (
          <>
            {record.recruit.child.address} {record.recruit.child.addressDetail}
          </>
        ) : (
          <>-</>
        ),
    },
    {dataIndex: 'meetingDate', key: 'meetingDate', title: '미팅일시', valueType: 'text'},
    {dataIndex: 'etc1', key: 'etc1', title: '기타1', valueType: 'text'},
    {dataIndex: 'etc2', key: 'etc2', title: '기타2', valueType: 'text'},
    {dataIndex: 'etc3', key: 'etc3', title: '기타3', valueType: 'text'},
  ...
  return (
    <BaseProTable<any, any>
      ...
      request={async (...p) => {
        ...
        console.log(params)
        console.log(p)
        const output: any = await getRecruitMeetList({...params});
